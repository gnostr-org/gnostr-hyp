...
Examples:
  ...
  gnostr-hyp drive http hyper://1234..af/
`
...
export default {
  ...
  command: async function (args) {
    ...
    const drive = await HyperStruct.get(args._[0], { expect: 'hyperdrive' })
    ...
    process.on('SIGINT', () => {
      ...
      const to = setTimeout(() => process.exit(0), 1e3)
      to.unref()
    })
  }
}
  ...
  return async function (req, res) {
    ...
    try {
      ...
      if (entry && entry.isDirectory()) {
        ...
        const files = await drive.api.promises.readdir(filepath)
        files.sort()
        if (files.includes('index.html')) {
          ...
          res.writeHead(200, { 'Content-Type': 'text/html' })
          const updog = filepath === '/' ? '' : '<li><a href="..">..</a></li>'
      ...
      let mimeType = entry.metadata.mimeType
      if (!mimeType) {
