...
Examples:
  ...
  gnostr-hyp drive diff hyper://1234..af/ ./local-folder
  gnostr-hyp drive diff ./local-folder hyper://1234..af/remote-folder --no-delete
  gnostr-hyp drive diff hyper://1234..af/ hyper://fedc..21/ --commit
`
...
export default {
  ...
  command: async function (args) {
    ...
    const left = await toDftParam(args._[0])
    const right = await toDftParam(args._[1])
    ...
    for (const item of diff) {
      const path = item.type === 'dir' ? item.path + '/' : item.path
      if (item.change === 'add') {
