...
Examples:
  ...
  gnostr-hyp daemon status
`

export default {
  name: 'daemon status',
  description: 'Check the status of the gnostr-hyp daemon.',
  usage: {
    simple: '',
    full: FULL_USAGE
  },
  command: async function (args) {
    try {
      ...
      console.error(chalk.bold(`gnostr-hyp ${versionString}`))
      console.error(chalk.bold(`gnostr-hyp API v${st.apiVersion}`))
      console.error(`Your address: ${st.remoteAddress} (${st.holepunchable ? 'Hole-punchable' : 'Not Hole-punchable'})`)
    } catch {
      console.error(`gnostr-hyp daemon not active`)
    }
