...
export class HyperStructInfoTracker {
  ...
  genStatusLine () {
    const netCfg = this.genStatusNetCfg()
    if (!this.struct) {
  ...
  genStatusNetStats () {
    const stats = this.netStats
    if (!stats) return ''
    const dl = bytes(stats.download[stats.download.length - 1], { unitSeparator: ' ' })
    const ul = bytes(stats.upload[stats.download.length - 1], { unitSeparator: ' ' })
    return `[↓${dl}/s ↑${ul}/s]`
  }

  genStatusNetCfg () {
    let netCfg = 'Not swarming'
    if (this.netCfg) {
  ...
  async calcBlockState () {
    ...
    if (this.struct.type === 'hyperdrive') {
      ...
      const contentCore = await new Promise(resolve => this.struct.api.getContent((err, core) => resolve(core)))
      for (const entry of await this.struct.api.promises.readdir('/', { recursive: true, includeStats: true })) {
